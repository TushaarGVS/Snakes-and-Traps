...
# here we loop with respect to spaces ' '. 

declare -i height=30 width=60  # -i : integer.

# row and column numbers of head and tail.
...
border_color="\E[30;43m" # yellow! - echo -e "\E[30;43m"
snake_color="\E[32;42m"  # green!
food_color="\E[34;44m"	 # blue!
text_color="\E[31;43m"	 # shade of yellow!
no_color="\E[0m"			 # reset to no color!
...
init_game() {
    clear						# echo -n : no new line!
    echo -ne "\033[?25l" 	# turns off the cursor = setterm -cursor off.
    stty -echo				   # donot echo the terminal.
    for ((i=0; i<height; i++)); do
        for ((j=0; j<width; j++)); do
            eval "arr$i[$j]=' '"	  		#initialize board as a 2D array.
        done
    done
}

move_and_draw() {
    echo -ne "\E[${1};${2}H$3" 	# args: x-coordinate, y-coordinate and color.
}

# print everything in the buffer.
draw_board() {
    move_and_draw 1 1 "$border_color+$no_color" 	# '+' in the left-top corner.
    for ((i=2; i<=width+1; i++)); do					# '-' in the first row.
    ...
    for ((i=0; i<height; i++)); do						# '|' for the height.
...
move_snake() {
    ...
    local d=$(echo $body | grep -o '[0-3]$')  # grep -o: only matching.
...
getchar() {
    ...
    while true; do
        read -s -n 1 key  #reads 1 value in silent mode!
        case "$key" in
...
game_loop &		# creating a process.
game_pid=$!		# getting the pid of the background process.
getchar

clear_game
exit 0

# merge the background process with the current process and kill the background process.
# use `trap` and `kill` to do the same.
